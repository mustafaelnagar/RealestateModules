...
class Realestate_core extends CI_Controller {
...

	{
		...
		redirect(site_url('admin//'.$from.'/'.$page));		
...

	{
		...
		$this->config->load('idev_properties');
...

	{
		...
		//if($purpose=='PURPOSE_SALE' && $this->input->post('price_negotiable')!=1)
		...
		elseif($purpose=='PURPOSE_RENT') //&& $this->input->post('price_negotiable')!=1)
...
//			$this->form_validation->set_rules('rent_price_unit', 'Rent Price unit', 'required');								
...

	{
        ...
        $data['content'] = $this->load->view('admin//wordfilter_view',$value,TRUE);
...

	{
...

		{
			...
			redirect(site_url('admin//wordfilter'));			
