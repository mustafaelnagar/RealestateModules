...
class Reservation_core extends CI_Controller {
	...
	public function all($start='0')
	{
		 ...
		 $reservations	= $this->reservation_model->get_all_posts_by_range($start,$this->per_page,'invoice_date'); //order by invoice_date
		...
		$total 				= $this->reservation_model->count_all_reservatisons();
		
		$value['pages']		= configPagination('admin/reservation/all',$total,5,$this->per_page);

        
        $data['title'] = 'All Reservations';
        
        $data['content'] = $this->load->view('admin/reservation/allreservation_view',$value,TRUE);
...
//print_r($value);
	...
	public function manage($id='')
	{
		...
		if($id!='')
		{
			...
			$values['price']  = 'price';
			$values['page'] 		= $this->reservation_model->get_post_by_id($id);
		}
		...
		else
		{
			...
			else
			{
				if($this->input->post('action_type')=='update')
				{
					...
					$this->reservation_model->update_post($data,$id);
					$this->session->set_flashdata('msg', '<div class="alert alert-success">Post updated</div>');				
				}
				else
				{
					$id = $this->reservation_model->insert_post($data);
	...
	public function delete($page='0',$id='',$confirmation='')
	{
		...
		else
		{
			if($confirmation=='yes')
			{
				...
				{
					$this->reservation_model->delete_post_by_id($id);
