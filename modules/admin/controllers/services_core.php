...
class Services_core extends CI_Controller {
	...
	public function __construct()
	{
		...
		echo "All services Core  <br/>";//debugging
		...
		echo "P value <br/>";//debugging

		$this->per_page = get_per_page_value();#defined in auth helper

		echo "Model value <br/>";
		
		// $resultofcall= $this->load->model('Services_model');
		// $resultofcall= $this->load->model('test_m');

		// echo "the result of call is ". $resultofcall."<br/>";//debugging
...
$this->load->model('services_model');
		...
		$value['posts']  	= $this->services_model->get_all_posts_by_range($start,$this->per_page,'create_time');
		print_r($value);
		$total 				= $this->services_model->count_all_posts();
		// $value['pages']		= configPagination('admin/blog/all',$total,5,$this->per_page);
        $data['title'] = 'All Services';
print_r($data);
        $data['content'] = $this->load->view('admin/services/allposts_view',$value,TRUE);
	...
	public function manage($id='')
	{
		...
		if($id!='')
		{
			...
			$values['page'] 		= $this->services_model->get_post_by_id($id);
		}
	...
	public function add()
	{
		...
		else
		{
			...
			// $data['descridescription'] 	= $this->input->post('description');
			...
			else
			{
				if($this->input->post('action_type')=='update')
				{
					...
					$this->session->set_flashdata('msg', '<div class="alert alert-success">Service updated</div>');				
				}
				else
				{
					echo "saetrt";
					$id = $this->serv_model->insert_post($data);
					$this->session->set_flashdata('msg', '<div class="alert alert-success">Service Created</div>');				
				}				
			}	

			redirect(site_url('admin/serv/manage/'.$id));		
		}
	...
	public function delete($page='0',$id='',$confirmation='')
	{
		if($confirmation=='')
		{
			$data['content'] = $this->load->view('admin/confirmation_view',array('id'=>$id,'url'=>site_url('admin/services/delete/'.$page)),TRUE);
			$this->load->view('admin/template/template_view',$data);
		}
		else
		{
			if($confirmation=='yes')
			{
				...
				{
					$this->services_model->delete_post_by_id($id);
