...
<div id="trainingrecords-attachments" target=".trainingrecords-attachment-list">
	...
	<form action="<?php echo site_url('admin/cms/upload_cms_photo');?>" method="post" enctype="multipart/form-data">
	...
	jQuery('#'+tab+' > form').ajaxForm({
		...
		complete: function(xhr) {
...

			{
				...
				var base_url = "<?php echo base_url();?>uploads/cmsuploads/thumb/";

				
				//setting the values back to the end user 

				window.parent.jQuery('#service_path_2').attr("src",base_url+response.name);

				window.parent.jQuery('#service_photo_2').attr("value",response.name);
